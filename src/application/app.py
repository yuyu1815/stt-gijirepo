...
class Application:
    ...
    def _process_file(self, file_path: Path, args: Dict) -> Dict:
        ...
        try:
            ...
            # 動画分析モード
            if args.get("video_analysis", False) and media_file.is_video and not media_file.is_dark_video:
                return self._process_video_analysis(media_file, args)
            ...
            if media_file.is_video and not media_file.is_dark_video:
                # 重要シーンを検出して画像を抽出
                important_scenes = video_analysis_service._detect_important_scenes(media_file)
                extracted_images = video_analysis_service._extract_images_from_scenes(media_file, important_scenes)
                
                # 動画分析（軽量版）
                if args.get("analyze_video", False):
                    video_analysis_result = video_analysis_service._generate_mock_analysis(media_file, extracted_images)
    ...
    def _process_video_analysis(self, media_file: MediaFile, args: Dict) -> Dict:
        """
        動画分析モードで処理
        ...
        Returns:
            処理結果の辞書
        """
        logger.info(f"動画分析モードで処理します: {media_file.file_path}")
        
        try:
            # 動画分析
            analysis_result = video_analysis_service.analyze_video(media_file)
            
            # 分析に失敗した場合
            if "error" in analysis_result:
                logger.error(f"動画分析に失敗しました: {media_file.file_path} - {analysis_result['error']}")
                return {
                    "success": False,
            ...
            logger.error(f"動画分析中にエラーが発生しました: {media_file.file_path} - {e}")
            return {
                "success": False,
                "file_path": str(media_file.file_path),
                "error": str(e)
