...
class HallucinationService:
    ...
    def _check_with_gemini(self, file_path: Path, transcription_text: str, prompt: str) -> str:
        ...
        # 実際の実装では、Gemini APIクライアントを使用して音声ファイルと文字起こしテキストを送信し、
        # ハルシネーションチェック結果を取得する
        ...
        while retry_count <= self.max_retries:
            try:
                # ここに実際のAPI呼び出しコードを実装
                # 例: response = gemini_client.check_hallucination(file_path, transcription_text, prompt)
                
                # モック応答（実際の実装では削除）
                mock_response = self._generate_mock_hallucination_check(file_path, transcription_text)
    ...
    def _generate_mock_hallucination_check(self, file_path: Path, transcription_text: str) -> str:
        """
        モックハルシネーションチェック結果を生成（実際の実装では削除）
        ...
        for i, line in enumerate(lines):
            ...
            if random.random() < 0.1:
                ...
                results.append(f"SEGMENT: {line}")
                results.append(f"SEVERITY: {severity}")
                results.append(f"REASON: これはモックのハルシネーション検出です。実際の実装では、音声と文字起こしを比較して判断します。")
                results.append(f"CORRECTED: {line.split(': ', 1)[0] if ': ' in line else ''}: これは修正されたテキストです。")
                results.append("")
            
        if not results:
            results.append("ハルシネーションは検出されませんでした。文字起こし結果は信頼性が高いと判断されます。")
