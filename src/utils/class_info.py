...
def get_class_info(file_path: str, settings_file: str = "settings.json") -> Dict[str, Any]:
    ...
    try:
        ...
        logger.warning(f"時間割の読み込みに失敗しました: {str(e)}")
    ...
    if date_time_match:
        ...
        try:
            ...
            if weekday >= 5:  # 5:土曜日, 6:日曜日
                logger.info(f"注意: {day_of_week}は授業がありません")
                return {"name": "不明", "teacher": "不明", "datetime": datetime_str}
        except (ValueError, IndexError) as e:
            logger.warning(f"日付の解析エラー: {e}")
            return {"name": "不明", "teacher": "不明", "datetime": datetime_str}
...
def extract_class_period(time_minutes: int) -> Optional[str]:
    """
    時間（分）から授業時限を取得
